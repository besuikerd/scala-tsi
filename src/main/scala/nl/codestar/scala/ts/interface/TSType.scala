...
@implicitNotFound(
  "Could not find a Typescript type mapping for type ${T}.\nMake sure an implicit TSType[${T}] is in scope and was defined before this point.")
trait TSType[T] { self =>
...
@implicitNotFound(
  "Could not find a named typescript type - TSNamedType[${T}] - in scope.\nIf there is a TSType[${T}], here the typescript type must have a name")
...
@implicitNotFound(
  "Could not find an typescript interface type - TSIType[${T}] - in scope.\nIf there is a TSType[${T}], here we can only use a typescript interface.")
